...
`deploy-openstack` 是一个基于 Python 3 的单机 OpenStack Antelope 自动化部署项目，目标环境为 Ubuntu 22.04 LTS，适合实验室、PoC、内部测试和学习场景。

这次优化主要聚焦三件事：

- 改善入口脚本，支持步骤编号、步骤 ID 和命令行参数
- 抽取公共运行时能力，统一配置校验、日志和命令执行
- 补全中英文文档，方便交付、维护和二次修改

## 1. 项目范围

当前支持：

- 单机版 OpenStack Antelope
- Ubuntu 22.04.x LTS
- Python 3
...
安装 Ubuntu 22.04 LTS，保证系统能联网并访问软件源。
...
如果仓库里同时维护 `main`、`Epoxy` 之类的分支，建议把这次的运行时优化和文档更新作为一组提交统一同步。这样可以保证不同分支的部署入口、配置校验和使用文档保持一致，后续排障也会更轻松。
