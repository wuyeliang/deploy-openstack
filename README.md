...
`deploy-openstack` is a Python 3 automation project for deploying a single-node OpenStack Antelope environment on Ubuntu 22.04 LTS. The repository focuses on a simple step-by-step installation flow for the controller services used in a lab, PoC, or internal test environment.

This repository has been optimized around three goals:
...
- more complete documentation for installation, operation, and troubleshooting

## 1. Scope

Supported deployment model:

- single-node OpenStack Antelope
- Ubuntu 22.04.x LTS
- Python 3
...
Install Ubuntu 22.04 LTS and ensure the node can access package mirrors and the internet.
...
## 8. Branch Sync Guidance

If you maintain multiple branches such as `main` and `Epoxy`, keep the shared runtime and documentation changes together in one commit set. Then sync them branch by branch with `cherry-pick` or a targeted merge so the deployment behavior stays aligned across maintained branches.

## 9. Access After Deployment
